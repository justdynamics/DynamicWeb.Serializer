...
- 2026-05-01: Phase 41 added — Phase 40 admin-UI polish + cross-page consistency. Five issues surfaced during manual verification of Phase 40's live-host deploy: (1) tree node "Item Types" → "Item Type Excludes" rename; (2) sanity-check empty Deploy-baseline excludeFieldsByItemType (likely lost during 40-04 flat-shape rewrite); (3) eCom_CartV2 list-vs-detail dual-list mismatch — list shows 21 excluded, detail page shows 0; (4) "Sample XML from database" editor on XmlTypeEditScreen needs to fill the screen by default; (5) Mode dropdown screen error caused by parenthetical option labels ("Deploy (source-wins)" / "Seed (field-level merge)") — drop the suffix. All admin-UI / dev-experience scoped — no runtime config-shape changes.
