...
**Success Criteria** (what must be TRUE):
  ...
  6. Inspecting either manifest with a JSON viewer shows the discriminator (`providerType`) at position 0 of every entry object; hand-reordering the discriminator below another property in a fixture and re-reading still produces a loud typed error (`JsonException` *or* `NotSupportedException` — .NET 8 STJ throws the latter because `ManifestEntry` is abstract; future STJ may switch to `JsonException`, and the SC-6 test accepts both) — never a silent fallthrough to base-type binding with the discriminator value lost. The closed-set allow-list (`IgnoreUnrecognizedTypeDiscriminators=false`, `UnknownDerivedTypeHandling=FailSerialization`) plus the unknown-discriminator-value guard test pin the same invariant from a second angle.
...
Plans:
- [x] 44-01-PLAN.md — Zip-import convergence (D-01..D-04) + Layer B test port (CONVERGE-03) + [Obsolete] overload deletion (CONVERGE-04) + schedule-task ratification (CONVERGE-05) + Phase 43 REVIEW fold-in (CONVERGE-07: WR-02..04, IN-01..03, IN-06); 7 atomic commits per D-05 ratchet
