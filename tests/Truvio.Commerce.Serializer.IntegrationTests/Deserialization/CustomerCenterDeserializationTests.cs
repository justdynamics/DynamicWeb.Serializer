...
// Execution requires:
//   1. Build: dotnet build src/Truvio.Commerce.Serializer/ -c Debug
//   2. Copy DLLs to Swift2.1 bin:
//        copy src\Truvio.Commerce.Serializer\bin\Debug\net8.0\Truvio.Commerce.Serializer.dll
...
//   4. Run: dotnet test tests/Truvio.Commerce.Serializer.IntegrationTests/ --filter "Category=Integration"
...
namespace Truvio.Commerce.Serializer.IntegrationTests.Deserialization;
...
public class CustomerCenterDeserializationTests : IDisposable
{
    ...
    public CustomerCenterDeserializationTests()
    {
        _outputDir = Path.Combine(Path.GetTempPath(),
            "SerializerDeserTest_" + Guid.NewGuid().ToString("N")[..8]);
    ...
    private static SerializerConfiguration BuildConfig(string outputDir, int areaId, string pagePath)
    {
    ...
    public void Verify_PageUniqueId_PreservedOnInsert()
    {
        ...
        testPage.MenuText = "Serializer_GuidTest_" + knownGuid.ToString("N")[..8];
        ...
        finally
        {
            ...
            try
            {
                var cleanup = Services.Pages.GetPagesByParentID(page.ID)
                    .FirstOrDefault(p => p.MenuText?.StartsWith("Serializer_GuidTest_") == true);
                if (cleanup != null)
